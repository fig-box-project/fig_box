...
def test_route(bp: APIRouter, test_auth):
    ...
    @bp.get('/packages')
    def packages():
        rt = list(PackageManager.get_packages_set())
        rt.sort()
        return rt

    @bp.get('/pkg_install')
    def pkg_install(pkg: str, imp_str: Optional[str] = None):
        PackageManager.install_package(pkg, imp_str)
    ...
    # async def download():
    #     from app.core.assets.input_assets_connector.InputDownloadConnector import InputDownloadConnector
