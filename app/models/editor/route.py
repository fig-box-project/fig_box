...
from ..assets.input_assets_connector import InputZipDirConnector
...
def editor_route(bp):
    ...
    @bp.get('/packup', description='打包下载')
    async def packup_templates():
        conn = InputZipDirConnector('packup', 'files.zip', ['files/templates'])
        await conn.packup()
        return conn.get_full_url()
